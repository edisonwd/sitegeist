...
import { streamSimple } from "../utils/stream.js";
...
export class AgentInterface extends LitElement {
		...
		this._unsubscribeSession = this.session.subscribe(async (ev: AgentEvent) => {
			switch (ev.type) {
				...
				case "agent_end":
					...
					// finishRun() sets isStreaming=false after this event fires
					// but doesn't emit a notification. Force it here so the
					// MessageEditor renders the Send button instead of the Stop button.
					if (this.session) {
						(this.session.state as any).isStreaming = false;
					}
					this.requestUpdate();
					break;
				case "message_update":
	...
	public async sendMessage(input: string, attachments?: Attachment[]) {
		...
		// Check if API key exists for the provider (check both built-in and custom providers)
		const provider = session.state.model.provider;
		const storage = getAppStorage();
		const apiKey = await storage.providerKeys.get(provider);

		// Check if this is a custom provider (custom providers may not require API keys)
		const customProviders = await storage.customProviders.getAll();
		const isCustomProvider = customProviders.some((p) => p.name === provider);

		// If not a custom provider and no API key, prompt for it
