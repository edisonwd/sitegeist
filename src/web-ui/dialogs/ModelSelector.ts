		...
		try {
			...
			for (const provider of customProviders) {
				...
				if (isAutoDiscovery) {
					...
					// Manual provider - ensure provider name is current so the
					// allowedProviders filter cannot drop models with a stale provider field
					const modelsWithProvider = provider.models.map((model: Model<any>) => ({
						...model,
						provider: provider.name,
		...
		try {
			...
			for (const provider of knownProviders) {
				try {
					...
					// Skip provider if model listing fails
				}
			}
		} catch (_e) {
			// Skip known providers entirely if catalog lookup fails
		}
