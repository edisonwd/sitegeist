...
export class ProvidersModelsTab extends SettingsTab {
	...
	getTabName(): string {
		return i18n("Custom Model Providers");
	}
		...
		return html`
			<div class="flex flex-col gap-6">
				<div class="flex items-center justify-between">
					<div>
						<h3 class="text-sm font-semibold text-foreground mb-2">${i18n("Custom Model Providers")}</h3>
						<p class="text-sm text-muted-foreground">
							${i18n("Configure servers with auto-discovered or manually defined models.")}
						</p>
					</div>
					${Select({
	...
	private async deleteProvider(provider: CustomProvider) {
		if (!confirm(i18n("Are you sure you want to delete this provider?"))) {
			return;
		}
